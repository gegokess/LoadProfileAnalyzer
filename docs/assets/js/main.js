...
class LoadAnalyzer {
  ...
  initializeEventListeners() {
    ...
    const confirmBtn = document.getElementById("timestamp-confirm");
    if (confirmBtn) {
      confirmBtn.addEventListener(
        "click",
        this.handleTimestampConfirm.bind(this),
      );
    }
  }
  ...
  handleTimestampConfirm() {
    ...
    this.dateColumn = dateSelect.value;
    this.timeColumn = timeSelect.value || null;

    try {
      this.data = this.validateAndProcessData(
        this.rawData,
        this.dateColumn,
        this.timeColumn,
      );
  ...
  validateAndProcessData(rawData, dateColumn, timeColumn) {
    ...
    if (!dateColumn) {
      const guess = this.findTimestampColumns(headers);
      dateColumn = guess.timestamp || guess.date;
      timeColumn = guess.timestamp ? null : guess.time;
    }

    if (!dateColumn) {
      throw new Error("Keine Datumsspalte gefunden.");
    }

    const loadColumns = headers.filter(
      (h) =>
        h !== dateColumn &&
        h !== timeColumn &&
        this.isNumericColumn(rawData, h),
    );
  ...
  findTimestampColumns(headers) {
    const tsPatterns = ["timestamp", "datetime", "zeitstempel"];
    const datePatterns = ["date", "datum"];
    const timePatterns = ["time", "uhrzeit"];

    const timestamp = headers.find((h) =>
      tsPatterns.some((p) => h.toLowerCase().includes(p)),
    );
    const date = headers.find((h) =>
      datePatterns.some((p) => h.toLowerCase().includes(p)),
    );
    const time = headers.find((h) =>
      timePatterns.some((p) => h.toLowerCase().includes(p)),
    );
  ...
  parseTimestamp(dateStr, timeStr) {
    ...
    let ts = new Date(combined);
    if (!isNaN(ts.getTime())) return ts;

    const regexDMY =
      /^(\d{1,2})[.\/\-](\d{1,2})[.\/\-](\d{2,4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?/;
    let m = combined.match(regexDMY);
    if (m) {
      let [, d, mth, y, h = 0, min = 0, s = 0] = m;
    ...
    const regexMDY =
      /^(\d{1,2})\/(\d{1,2})\/(\d{2,4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?/;
    m = combined.match(regexMDY);
    if (m) {
      let [, month, day, y, h = 0, min = 0, s = 0] = m;
      if (y.length === 2) y = "20" + y;
      return new Date(y, month - 1, day, h, min, s);
    }
  ...
  validateTimestampSequence(data) {
    ...
    if (gaps.length > data.length * 0.1) {
      console.warn(
        "Zeitstempel-Reihenfolge ist nicht vollständig monoton steigend.",
      );
    }
  }
  ...
  calculateTotalConsumption(column) {
    ...
    return this.data.data.reduce(
      (sum, row) => sum + row[column] * intervalHours,
      0,
    );
  }
  ...
  estimateIntervalHours() {
    ...
    const interval =
      (this.data.data[1].timestamp - this.data.data[0].timestamp) /
      (1000 * 60 * 60);
  ...
  createHeatmapData(column) {
    ...
    Object.keys(hourlyData).forEach((key) => {
      ...
      const average =
        hourlyData[key].reduce((sum, val) => sum + val, 0) /
        hourlyData[key].length;
  ...
  calculateDailyProfiles(column) {
    ...
    return {
      weekday: weekdayData.map((hourData) =>
        hourData.length > 0
          ? hourData.reduce((sum, val) => sum + val, 0) / hourData.length
          : 0,
      ),
  ...
  updatePeakShaving() {
    ...
    const sampledData = this.data.data.filter(
