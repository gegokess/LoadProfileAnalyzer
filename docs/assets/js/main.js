...
class LoadAnalyzer {
  ...
  setupColumnSelection() {
    ...
    this.data.columns.forEach((col) => {
      ...
      div
        .querySelector("input")
        .addEventListener("change", this.handleColumnSelection.bind(this));
    });
  ...
  generateLoadDurationCurve() {
    ...
    const datasets = this.selectedColumns.map((col, i) => {
      const vals = this.resampled
        .map((r) => r[col])
        .filter((v) => v > 0)
        .sort((a, b) => b - a);
      return {
        label: col,
        data: vals.map((v, idx) => ({ x: (idx / vals.length) * 100, y: v })),
    ...
    this.charts.loadDuration = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: "Lastdauerkurve" },
          legend: { display: this.selectedColumns.length > 1 },
        },
        scales: {
          x: {
  ...
  generateHistogram() {
    ...
    this.charts.histogram = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: `Leistungsverteilung - ${col}` },
  ...
  createHistogramBins(vals, numBins) {
    ...
    for (let i = 0; i < numBins; i++) {
      const lo = min + w * i,
        hi = lo + w;
      const cnt = vals.filter(
        (v) => v >= lo && (i === numBins - 1 ? v <= hi : v < hi),
      ).length;
      bins.push({ min: lo, max: hi, count: cnt });
    }
    return bins;
  }
  ...
  generateRealHeatmap() {
    ...
    const col = this.selectedColumns[0];
    const points = this.resampled.map((r) => {
      const d = r.timestamp;
      const day = (d.getDay() + 6) % 7;
      const hour = d.getHours();
      return { x: day, y: hour, v: r[col] };
    });
        ...
        datasets: [
          {
            ...
            backgroundColor: (ctx) => {
              ...
              const alpha = v / maxV;
              return `rgba(59,130,246,${0.3 + 0.7 * alpha})`;
            },
      ...
      options: {
        plugins: {
          tooltip: {
            callbacks: {
              title: (ctx) =>
                `Tag: ${["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"][ctx[0].parsed.x]}, Stunde: ${ctx[0].parsed.y}:00`,
              label: (ctx) => `Wert: ${ctx[0].raw.v.toFixed(2)}`,
            },
          },
        },
        scales: {
          x: {
            ...
            max: 6,
            ticks: {
              callback: (i) => ["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"][i],
            },
          },
  ...
  generateMonthlyChart() {
    ...
    this.resampled.forEach((r) => {
      const m = r.timestamp.getMonth();
      this.selectedColumns.forEach((col) => {
        monthly[m][col] = (monthly[m][col] || 0) + (r[col] * interval) / 1000;
      });
    });
    const datasets = this.selectedColumns.map((col, i) => ({
      label: col,
      data: monthly.map((m) => m[col] || 0),
      backgroundColor: this.getColor(i, 0.7),
      borderColor: this.getColor(i),
      borderWidth: 1,
    }));
    this.charts.monthly = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: "Monatsverbrauch" },
          legend: { display: this.selectedColumns.length > 1 },
        },
        scales: {
  ...
  generateBoxplots() {
    ...
    this.resampled.forEach((r) =>
      monthlyData[r.timestamp.getMonth()].push(r[col]),
    );
    const stats = monthlyData.map((data) => {
      if (!data.length) return { min: 0, q1: 0, median: 0, q3: 0, max: 0 };
      const s = [...data].sort((a, b) => a - b);
      return {
        min: s[0],
        q1: this.calculatePercentile(s, 25),
        median: this.calculatePercentile(s, 50),
        q3: this.calculatePercentile(s, 75),
        max: s[s.length - 1],
      };
    });
    const datasets = [
    ...
    this.charts.boxplot = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: `Monatliche Verteilung - ${col}` },
          legend: { filter: (item) => item.text !== "" },
        },
        scales: {
          x: { title: { display: true, text: "Monat" } },
  ...
  generateDailyProfile() {
    ...
    const col = this.selectedColumns[0];
    const wd = Array.from({ length: 24 }, () => []),
      we = Array.from({ length: 24 }, () => []);
    this.resampled.forEach((r) => {
      const h = r.timestamp.getHours();
      (r.timestamp.getDay() >= 1 && r.timestamp.getDay() <= 5 ? wd : we)[
        h
      ].push(r[col]);
    });
    const getAvg = (arr) =>
    ...
    this.charts.dailyProfile = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: `Tagesprofil-Vergleich - ${col}` },
        },
        scales: {
  ...
  generatePeakShaving() {
    ...
    const threshold =
      (document.getElementById("peak-shaving-slider").value / 100) * maxVal;
    const interval = this.estimateIntervalHours();
    const savings = vals.reduce(
      (sum, v) => sum + (v > threshold ? (v - threshold) * interval : 0),
      0,
    );
    document.getElementById("peak-shaving-savings").textContent =
      `${savings.toFixed(1)} kWh`;
    ...
    this.charts.peakShaving = new Chart(ctx, {
      type: "line",
      data: {
        labels: sample.map((_, i) => i),
        datasets: [
          {
            label: "Originallast",
            data: sample.map((r) => r[col]),
          ...
          {
            label: "Gekappte Last",
            data: sample.map((r) => Math.min(r[col], threshold)),
          ...
          },
          {
            ...
            data: Array(sample.length).fill(threshold),
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: `Peak-Shaving-Szenario - ${col}` },
        },
        scales: {
          x: { display: false },
          y: { title: { display: true, text: "Leistung (kW)" } },
  ...
  estimateIntervalHours() {
    ...
    const delta =
      (this.resampled[1].timestamp - this.resampled[0].timestamp) /
      (1000 * 3600);
